...
export function FavouriteArtists({ favouriteArtists, followedArtists, onToggle }: Props) {
  const artistsByPopularity = favouriteArtists.sort((a, b) => b.popularity - a.popularity)
  ...
  return (
    <ul className="styleless favourite-artists">
      {top50artists.map((spotifyArtist) => {
        ...
        const isFollowed = followedArtists.some((followedArtist) => followedArtist.id === spotifyArtist.id)
